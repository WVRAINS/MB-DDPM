...
zero.count.by.taxa = apply(Y.count.case, 2, function(x) {
  sum(x == 0)
})
...
nsimu = 1000
set.seed(2174171)
Y.NorTA.case = synth_comm_from_counts(
  comm = Y.count.case.filter,
  mar = 2,
  distr = 'zinegbin',
  n = nsimu
)
...
case.compo.NorTA.spec = t(apply(case.count.NorTA.spec, 1, function(x) {
  x / sum(x)
}))
...
zero.count.by.taxa = apply(Y.count.ctrl, 2, function(x) {
  sum(x == 0)
})
...
nsimu = 1000
set.seed(2174171)
Y.NorTA.ctrl = synth_comm_from_counts(
  comm = Y.count.ctrl.tmp,
  mar = 2,
  distr = 'zinegbin',
  n = nsimu
)
...
# convert to the compositional data #
ctrl.compo.NorTA.spec = t(apply(ctrl.count.NorTA.spec, 1, function(x) {
  x / sum(x)
}))
